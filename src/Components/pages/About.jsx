    ...
    <div className="min-h-screen bg-gradient-to-br from-teal-50 to-blue-100 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12 header-animation">
          <h1 className="text-3xl sm:text-4xl font-extrabold text-teal-800 font-poppins mb-6">About HomeMaid</h1>
          <img src={maidsGroup} alt="Team HomeMaid" className="h-96 object-cover mx-auto rounded-lg mb-8 border-2 border-teal-200 shadow-md" />
        ...
        <div className="bg-white p-6 rounded-2xl shadow-xl hover:shadow-2xl mb-12 transition-all duration-300 border border-teal-100">
          <h2 className="text-2xl font-bold text-teal-800 font-poppins mb-4">Our Mission</h2>
          <p className="text-gray-800 font-poppins mb-4">At HomeMaid, we’re committed to delivering exceptional home services with a focus on quality, affordability, and customer satisfaction. Our journey includes:</p>
        ...
        <div>
          <h2 className="text-2xl font-bold text-teal-800 font-poppins text-center mb-6">Meet Our Team</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {team.map((member, index) => (
              <div key={index} className="bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 text-center animate-card">
                <img src={member.image} alt={member.name} className="w-32 rounded-lg mx-auto mb-4 border-2 border-teal-200" />
                <h3 className="text-xl font-semibold text-teal-800 font-poppins mb-2">{member.name}</h3>
                <p className="text-gray-600 font-poppins mb-1">{member.role}</p>
                <p className="text-gray-500 font-poppins">{member.experience}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="text-center mt-12">
          <Link to="/services" className="inline-block bg-teal-600 text-white p-3 rounded-lg font-poppins text-sm sm:text-base hover:bg-teal-700 transition-all duration-300 shadow-md hover:shadow-lg transform hover:scale-105">
